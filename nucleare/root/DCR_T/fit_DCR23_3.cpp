...
void fit_DCR23_3(){
    ...
    cst->SetGrid();
    TGraph* gst = graph_filler("Staircase plot, G_{ampl}=35dB, V_{BIAS}=55V, T=23.3^{o}C;V_{thr}[mV];#nu[kHz]",stairpath+temp+"c.txt");
    cst->SetLogy();
...
//			LOW
    ...
    c17->SetGrid();
    c17->cd();
    TH1D* h17 = histo_filler("h17","Impulsi ricevuti a ??V_{thr}=-17mV con gate = 500ms, T=23.3^{o}C;N. impulsi/gate [x500 ms^{-1}];Conteggi [#]",DCR+"low_"+temp+"C.txt");
    gStyle->SetOptStat(0);
    gStyle->SetErrorX(0);
    h17->Draw("HIST E1");
...
//			HIGH
    ...
    c44->SetGrid();
    c44->cd();
    TH1D* h44 = histo_filler("h44","Impulsi ricevuti a V_{thr}=-44mV con gate = 500ms, T=23.3^{o}C;N. impulsi/gate [x500 ms^{-1}];Conteggi [#]",DCR+"high_"+temp+"C.txt");
    gStyle->SetOptStat(0);
    gStyle->SetErrorX(0);
    h44->Draw("HIST E1");
